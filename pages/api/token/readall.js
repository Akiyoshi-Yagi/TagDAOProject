...
const getAllTokens = async(req, res) => {
    try{
        ...
        try {
            new web3.eth.Contract(contractAbi, contractAddress).methods.tokenList().call().then(allTokens => {
                console.log(allTokens);
                return res.status(200).json({message: "fetch success", allTokens: allTokens});
            });
        } catch (error) {
            console.log(error);
